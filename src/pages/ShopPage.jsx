    ...
    <div className="container mx-auto px-4 py-6 lg:py-8 space-y-6">
      ...
      <div className="bg-gradient-to-r from-[#F5F5F5] via-white to-[#F5F5F5] p-5 lg:p-8 rounded-3xl border border-[#ECE7E8] flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <Filter size={14} className="text-[#000000]" /> Filters
          </button>
        ...
        <aside className="hidden lg:block lg:col-span-3 bg-white rounded-3xl border border-[#ECE7E8] p-6 space-y-6 sticky top-24 shadow-sm">
          <div className="flex items-center justify-between border-b pb-3">
            <h3 className="text-base font-bold text-gray-900 flex items-center gap-2">
              <SlidersHorizontal size={18} className="text-[#000000]" /> Filters
            </h3>
            <button onClick={resetFilters} className="text-xs text-[#000000] hover:underline font-semibold flex items-center gap-1">
            ...
            <div className="space-y-1">
              <button 
                ...
                className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition flex justify-between ${
                  selectedCategory === 'all' ? 'bg-[#F5F5F5] text-[#000000] font-bold' : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
              ...
              {CATEGORIES.map(c => (
                <button 
                  ...
                  className={`w-full text-left px-3 py-1.5 rounded-lg text-xs font-medium transition flex justify-between ${
                    selectedCategory === c.id ? 'bg-[#F5F5F5] text-[#000000] font-bold' : 'text-gray-600 hover:bg-gray-50'
                  }`}
                >
                  <span>{c.name}</span>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm lg:hidden animate-fade-in" onClick={() => setMobileFilterOpen(false)}>
          <div className="w-4/5 max-w-xs bg-white h-full p-6 flex flex-col justify-between overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                ...
                <div>
                  ...
                  {CATEGORIES.map(c => (
                    <button 
                      ...
                      className={`block w-full text-left py-1.5 text-xs ${selectedCategory === c.id ? 'font-bold text-[#000000]' : 'text-gray-700'}`}
                    >
                      {c.name}
                    </button>
                  ))}
            ...
            <button 
              ...
              className="w-full bg-[#000000] text-white py-3 rounded-full font-bold text-xs shadow-md mt-6"
            >
              Apply Filters
            </button>
          </div>
