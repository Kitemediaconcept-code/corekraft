...
export default function AccountPage({ userOrders, wishlist, onSelectProduct, onNavigateShop, onLogout }) {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl space-y-8">
      <div className="flex items-center gap-4 bg-gradient-to-r from-[#F5F5F5] to-white p-6 rounded-3xl border border-[#ECE7E8]">
        <div className="w-16 h-16 rounded-full bg-[#000000] text-white flex items-center justify-center font-bold text-2xl shadow-md">
          RM
        </div>
        <div>
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <div className="space-y-4">
                {mockOrders.map(order => (
                  <div key={order.id} className="p-4 rounded-2xl border border-gray-200 space-y-3 hover:border-[#E5E5E5] transition">
            ...
            <div className="space-y-6 animate-fade-in">
              ...
              <form onSubmit={handleTrackSearch} className="flex gap-2">
                <input 
                  ...
                  className="flex-1 text-xs p-3 rounded-xl border border-gray-200 outline-none focus:border-[#000000]"
                />
                <button type="submit" className="btn btn-primary btn-sm">Search</button>
              </form>

              {trackResult && (
                <div className="p-6 rounded-2xl bg-[#F5F5F5] border border-[#E5E5E5] space-y-4">
                  {trackResult.notFound ? (
                    ...
                    <>
                      <div className="flex justify-between text-xs font-bold text-gray-900">
                        <span>Order Status: <strong className="text-[#000000]">{trackResult.status}</strong></span>
                        <span>Carrier: {trackResult.carrier}</span>
                      </div>

                      {/* Timeline graphic */}
                      <div className="flex items-center justify-between text-[11px] pt-4">
                        <div className="text-center font-bold text-[#000000]">✓ Order Placed</div>
                        <div className="h-1 flex-1 bg-[#000000] mx-2"></div>
                        <div className="text-center font-bold text-[#000000]">✓ Logo Proof Approved</div>
                        <div className="h-1 flex-1 bg-[#000000] mx-2"></div>
